    ...
    def apply(obs: Observable[Notify]) = {
      ...
      try {
        ...
        case ObservableError(position, cause) =>
          val errMsg = s"Observable emitted an unexpected exception $cause - matched ${actions.take(position - 1)} and expected ${actions(position)}"
          MatchResult(matches = false, errMsg, errMsg)
        case ObservableClosed(0) =>
          val errMsg = s"Observable closed prematurely - expected ${actions(0)}"
          MatchResult(matches = false, errMsg, errMsg)
        case ObservableClosed(position) =>
          val errMsg = s"Observable closed prematurely - matched ${actions.take(position - 1)} and expected ${actions(position)}"
          MatchResult(matches = false, errMsg, errMsg)
        ...
        case TestFailed(position, action) =>
          val errMsg = s"Test matched ${actions.take(position - 1)} and then matching failed - received $action expected ${actions(position)}"
          MatchResult(matches = false, errMsg, errMsg)
      }
    }
  }
